...
syspath=r"C:\Users\DELL\PycharmProjects\Sentimental\static\\"
    ...
    if session['lg'] == "lin":
        if session['lg'] == "lin":
            if request.method=="POST":
                ...
                db.insert("insert into product(name,des, colour,price,quantity,store_id,photo,category_id) VALUES ('"+p+"','"+d+"', '"+clr+"','"+pr+"','"+q+"','"+str(session['lid'])+"','"+p1+"','"+ctg+"')")
...
@app.route('/and_view_purchase',methods=['post'])
def and_view_purchase():
    id=request.form['id']
    db=Db()
    # qry=db.select("select booking.bquantity*product.price as total_price,booking.*,product.*,booking_master.* from booking,product,booking_master where booking.master_id=booking_master.masterid and booking.product_id=product.id and booking_master.user_id='"+id+"' and booking_master.status='booked'")
    qry=db.select("select * from booking_master where user_id='"+id+"' and status='paid'")
    res={}
    if qry:
        res['status']="ok"
        res['data']=qry
...
@app.route('/and_view_purchase_list',methods=['post'])
def and_view_purchase_list():
    id=request.form['mid']
    db=Db()
    tot=0
    res=db.select("select booking.product_id, product.name, product.price, product.photo, booking.bquantity as qty, product.price* booking.bquantity as amount from booking, product where booking.product_id=product.id and booking.master_id='"+id+"'")
    for i in res:
        tot=tot+int(i['amount'])
...
def and_send_review():
    ...
    db=Db()
    db.insert("INSERT INTO `review` VALUES(NULL,'"+lid+"' ,CURDATE(), '"+pid+"', '"+ review + "')")
    return jsonify(status="ok")
...
def get_recommendation():
    ...
    if type == "colour":
        ...
        cnts=[]
        clrs=[]
        res=db.select("SELECT COUNT(booking_id) AS cnt, product.colour FROM product, `booking`, `booking_master`  WHERE `booking_master`.masterid=`booking`.master_id AND `booking`.product_id=`product`.id AND `booking_master`.user_id='"+lid+"' GROUP BY product.colour")
        for i in res:
            cnts.append(i['cnt'])
            clrs.append(i['colour'])
        mx=max(cnts)
        idx=cnts.index(mx)
        my_clr=clrs[idx]
        db=Db()
        res2=db.select("SELECT * FROM `product` WHERE colour='"+my_clr+"'")
        if len(res2) >0:
            return jsonify(status="ok", data=res2)
        else:
            return jsonify(status="no")
    elif type=="review":
        from prediction import predict
        db=Db()
        final=[]    #   for recommendation
        db=Db()
        ...
        for i in res:
            ...
            neg_cnt=0
            db=Db()
            res2=db.select("SELECT * FROM `review` WHERE pid='"+str(i['id'])+"'")
            for j in res2:
                rev=j['review']
                stat=predict(rev)
                if stat == "Positive":
            ...
            if pstv_cnt > neg_cnt:
                final.append(i)
        if len(final) >0:
            return jsonify(status="ok", data=final)
        else:
            return jsonify(status="no")
